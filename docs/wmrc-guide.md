...
`set_frame` 以外は成功で `true` / 失敗で `false` を返す（対象が見つからない・権限不足・
アプリが無応答など）。

| メソッド | 説明 |
|---|---|
| `WM.set_frame(window_id, x, y, w, h)` | **位置とサイズをまとめて設定**。戻り値は実際に落ち着いた矩形 `{"x"=>,"y"=>,"w"=>,"h"=>}`（失敗時 nil）。移動とリサイズを伴う場合は常にこちらを使う。 |
| `WM.move(window_id, x, y)` | ウィンドウ左上を (x, y)（top-left, グローバル）へ移動。 |
| `WM.resize(window_id, w, h)` | ウィンドウサイズを (w, h) に設定。 |
| `WM.focus(window_id)` | 指定ウィンドウへフォーカス（アプリ前面化＋キーウィンドウ確定＋重なり順）。`WM.raise_window` は同じものの別名。 |
...
#### なぜ `move` + `resize` ではなく `set_frame` か

`move` と `resize` を別々に呼ぶと、そのたびに対象アプリへの Accessibility 往復が発生する。
`set_frame` は 1 往復で済ませたうえで、次の 3 つを内部で面倒みる:

1. **「移動 → リサイズ → 再度移動」の順で当てる。** 先にリサイズすると、移動前のディスプレイの
   可視領域でクランプされて意図より小さくなることがある。またリサイズでウィンドウが画面内へ
   押し戻されるので、最後にもう一度位置を当て直す。
2. **書き込みの間だけ `AXEnhancedUserInterface` を落とす。** この属性が立っていると多くのアプリ
   （Electron / Chromium / JetBrains 系など）が位置・サイズ変更をアニメーション付きで遅延適用し、
   タイルが目に見えてガタつく。元から立っていた場合だけ、書き込み後に必ず戻す。
3. **結果を読み返して 1 回だけ再試行する。** それでも一致しない場合は、アプリ側の最小サイズ制約
   などが理由なので粘らない。**戻り値の実測矩形**を見れば「要求どおりにならなかった」ことが分かる。

```ruby
got = WM.set_frame(id, 0, 0, 800, 600)
# => {"x"=>0.0, "y"=>0.0, "w"=>800.0, "h"=>600.0}
# アプリの最小サイズに阻まれた場合は w/h が要求より大きいまま返る
```
...
内部で `WM.set_frame` を呼ぶので、戻り値は実際に落ち着いた矩形（失敗時 nil）。
...
- **ウィンドウ操作はタイムアウトしうる**。Accessibility の呼び出しは対象アプリの応答を待つため、
  相手がビジー／ハング／モーダル表示中だと返ってこない。ホストはアプリ（pid）ごとの専用スレッドで
  AX を実行し、**200ms で打ち切る**。打ち切られた呼び出しは `false`（`WM.set_frame` は `nil`）を返す。
  つまり **1 つのアプリが固まってもウィンドウマネージャ全体は止まらない**代わりに、
  **操作が黙って失敗しうる**。敷き詰めの結果を保証したいレシピは戻り値を確認すること。
  同じアプリが 3 回連続でタイムアウトすると以後は 25ms の打診に切り替わり、応答が戻れば通常運用へ復帰する。
