...
### 6. RPC（fd=3）はまだ未実証 ＋ 既知の落とし穴

`installRpcHooks`（Ruby→Swift の同期 RPC）は**本スパイクでは未検証**。設計上の重要な発見:

- **WASI の preopen は fd=3 から割り当てられる。** スパイクで `preopens: ["/": HOME]` を
  渡すと **fd 3 が preopen ディレクトリと衝突**する。`wm.rb` は `IO.new(3,"r+")` で
  RPC するので、**(a) preopen を使わず fd 3 を空ける**か、**(b) RPC fd を別番号**
  （例 100）に変える必要がある。`wm.rb` の `RPC_FD` と Swift 側で一致させること。
- フックの実装方針: `wasi.link` の**後に** `imports.define` で
  `wasi_snapshot_preview1.fd_write` / `fd_read`（＋ `fd_fdstat_get`）を上書きし、
  `fd==RPC_FD` のときだけ `WindowManagerCore.RpcChannel` に橋渡しする
  （iovec 走査は `Caller.instance.export("memory")` 経由で `Memory.withUnsafeMutableBufferPointer`）。
  それ以外の fd は既定 WASI へ委譲（委譲先 Function 参照の取り回しが要点 → 次セッションの de-risk 課題）。

## 次セッションへの示唆

1. **RubyVM.swift を本書 §3–5 の確定 ABI に合わせて実装**（生 C-API 前提の記述は破棄）。
   このコミットで骨格は更新済み。
2. **RPC ラウンドトリップ（fd hook）を Linux スパイクで de-risk**（§6 の fd 衝突に注意）。
   ここまで通れば `WM.windows` / キーリマップの縦切りが Mac 実機前にほぼ固められる。
3. **アーキ提案**: WasmKit が Linux でも動く以上、Ruby ランタイム層を
   `#if canImport(AppKit)` から外して**クロスプラットフォーム target に分離**すれば、
   eval＋RPC を **CI（Linux）でテスト**できる。プロジェクトの「Linux でテスト可能なコア」
   方針と完全に合致する。AppKit に触るのは実際のウィンドウ/キー dispatch だけ。
